...
`camera_extrinsics.json` / `intrinsics.json`，把相关数据集 GT label 投影到相机图上，画出
目标中心、3D 标注框线框、`#track_id 类别 距离` 辅助标注，帮助 VLM 把几何事实中点名的目标和图像实体对齐；
同时把 `gt_sdc_fut_traj` 投影成未来轨迹/箭头（黄色=前进，青色=后退），并按本车未来方向补充 FRONT/REAR/侧向相机，
...
- **teacher 深化版本**：在相机图上额外画 GT label 的 LiDAR 投影目标点、3D 标注框线框与 `#ID 类别 距离` 标签，并用方向箭头
  画本车 `gt_sdc_fut_traj` 未来轨迹（黄色=前进，青色=后退），再送给 Qwen3-VL。
