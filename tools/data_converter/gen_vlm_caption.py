...
_SYSTEM = (
    ...
    '3. 必须区分“运动状态”和“作业状态”：设备本体静止不等于空闲，但移动/行驶中的正面吊、'
    '轮胎吊、叉车、集装箱叉车默认只描述为移动/行驶，不要因为吊具、吊臂或叉臂抬起就判为'
    '正在装卸或等待作业。Crane 在本场景中是正面吊：正面吊静止、吊具/吊臂抬起并近距离正对'
    '集装箱、挂车或集卡时，应判为正在装卸作业，不要求已经接触箱体。其他设备只有目标静止，'
    ...
    '严禁用“障碍物”等笼统说法代替具体目标；\n'
    '5. 若事实中标注了某目标“（请判断作业状态）”，summary 必须只针对事实列表里点名的目标判断，'
    '四选一：正在装卸作业 / 等待作业 / 空闲 / 看不清。若该目标在给定相机图中不可见、被遮挡、'
    '或只来自相机兜底而非对应方位视角，必须写看不清或不提，不得补写图里没有的吊机/叉车状态。'
...
def _render_facts(facts):
    ...
    # crane_status is a loose diagnostic gate, not an authoritative fact.
    # Rendering its count encouraged the VLM to mention non-visible cranes.
    return '\n'.join(lines)
