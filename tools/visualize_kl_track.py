...
from visualize_kl_velocity import (boxes_to_numpy, compute_box_corners_bev,
                                   get_class_names, get_dataset_cfg,
                                   get_label_mapping, lidar_xy_to_display,
                                   load_points, map_label, tensor_to_numpy)
...
def reset_track_state(model) -> None:
    for name in ('test_track_instances', 'scene_token', 'timestamp',
                 'l2g_t', 'l2g_r_mat', '_test_track_instances',
                 '_test_prev_bev', '_test_scene_token'):
...
def unwrap_uniad_result(result):
...
def pred_arrays_from_uniad_result(result: dict, score_thr: float,
    ...
    num = min(len(boxes), len(scores), len(labels), len(track_ids))
    boxes = boxes[:num]
    scores = scores[:num]
    labels = labels[:num]
...
def gt_arrays_from_info(info: dict, dataset_cfg, class_names: Sequence[str],
                        label_mapping: Sequence[int]) -> Dict[str, np.ndarray]:
...
def build_uniad_model(cfg: Config, checkpoint: str, device: str, dataset):
...
def run_online(cfg: Config, args: argparse.Namespace) -> None:
    ...
    for frame_id, idx in enumerate(indices):
        ...
        pred_data = pred_arrays_from_uniad_result(
