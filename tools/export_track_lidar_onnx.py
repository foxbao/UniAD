...
class TrackLidarTRTWrapper(nn.Module):
    ...
    def forward(self, *inputs):
        return self.model.forward_track_lidar_trt(*inputs)


def parse_args():
    parser = argparse.ArgumentParser(
        description='Export base_track_lidar TensorRT boundary to ONNX.')
    parser.add_argument('config', help='LiDAR track TRT config path.')
    parser.add_argument(
        'checkpoint',
        nargs='?',
        default=None,
        help='Optional stage1_track_map_lidar checkpoint.')
    parser.add_argument(
        '--onnx-file',
        default='./onnx/base_track_lidar_trt.onnx',
...
def dump_inputs(inputs, output_dir):
    ...
    for name, value in zip(INPUT_NAMES, inputs):
        value.detach().cpu().numpy().tofile(os.path.join(
            output_dir, f'{name}.dat'))


def main():
    args = parse_args()
    if not torch.cuda.is_available():
        raise RuntimeError('LiDAR track ONNX export requires CUDA because '
    ...
    model.cuda().eval()
    wrapper = TrackLidarTRTWrapper(model).cuda().eval()
