...
class LidarPerceptionTransformer(BaseModule):
    ...
    @staticmethod
    def _ego_delta_numpy(meta: dict) -> np.ndarray:
        delta = meta['ego_motion_delta']
    ...
    def shift_from_queue_meta(self,
        ...
        for meta in queue_meta:
            if meta is None or not meta.get('prev_bev_exists', False):
                ...
                continue
            delta = self._ego_delta_numpy(meta)
            if self.rotate_prev_bev:
                translation = torch.as_tensor(
                    -delta[:2, 3], device=device, dtype=dtype)
                shifts.append(translation / extent)
            else:
                if abs(float(np.linalg.det(delta[:3, :3]))) < 1e-6:
                    shifts.append(torch.zeros(2, device=device, dtype=dtype))
                    continue
                prev_from_curr = np.linalg.inv(delta)
                translation = torch.as_tensor(
                    prev_from_curr[:2, 3], device=device, dtype=dtype)
                shifts.append(translation / extent)
    ...
    def get_bev_features_trt(self,
                             ...
                             use_prev_bev: torch.Tensor) -> torch.Tensor:
        """Tensor-only BEV encoder entry for ONNX/TensorRT export."""
        if shift is None:
...
@TRANSFORMER.register_module(force=True)
class LidarPerceptionTransformerTRTP(LidarPerceptionTransformer):
    """LiDAR PerceptionTransformer configured for TensorRT plugin export."""
