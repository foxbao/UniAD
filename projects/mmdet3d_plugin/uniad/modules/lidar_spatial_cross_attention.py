"""LiDAR BEV spatial cross-attention."""
...
class LidarSpatialCrossAttention(BaseModule):
    """Single-level deformable cross-attention over a LiDAR BEV feature map."""
    ...
    def forward(self,
        ...
        if spatial_shapes is None or reference_points is None:
            raise ValueError('SpatialCrossAttention requires '
                             'spatial_shapes and reference_points.')
        ...
        if reference_points.shape[-1] != 2:
            raise ValueError('SpatialCrossAttention expects 2D reference '
                             f'points, got shape {reference_points.shape}.')

        value = self.value_proj(value)
        if key_padding_mask is not None:
            value = value.masked_fill(key_padding_mask[..., None], 0.0)
        value = value.reshape(batch_size, num_value, self.num_heads, -1)

        sampling_offsets = self.sampling_offsets(query).view(
            batch_size, num_query, self.num_heads, self.num_levels,
            self.num_points, 2)
        attention_weights = self.attention_weights(query).view(
            batch_size, num_query, self.num_heads,
...
@ATTENTION.register_module(force=True)
class LidarSpatialCrossAttentionTRTP(LidarSpatialCrossAttention):
    """TensorRT-plugin export variant of LiDAR spatial cross-attention."""
