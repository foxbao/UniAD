...
from ..functions import multi_scale_deformable_attn

@ATTENTION.register_module(force=True)
class LidarTemporalSelfAttention(BaseModule):
    ...
    def forward(self,
        ...
        # ``value`` is flattened in batch-major queue order:
        # [b0_prev, b0_curr, b1_prev, b1_curr, ...].  The original BEVFormer
        # code uses ``value[:bs]``, which is only correct when bs == 1.  Select
        # the previous-BEV slot explicitly so multi-sample LiDAR training does
        # not mix current/history features across the batch.
        history_value = value.reshape(
            bs, self.num_bev_queue, num_value, embed_dims)[:, 0]
        query = torch.cat([history_value, query], dim=-1)
...
@ATTENTION.register_module(force=True)
class LidarTemporalSelfAttentionTRTP(LidarTemporalSelfAttention):
    """TensorRT-plugin export variant of LiDAR temporal self-attention."""
    ...
    def forward(self,
        ...
        sampling_offsets = sampling_offsets.reshape(
            bs * self.num_bev_queue, num_query, self.num_heads,
            self.num_levels * self.num_points * 2).contiguous()
        ...
        output = multi_scale_deformable_attn(
            value, spatial_shapes, reference_points, sampling_offsets,
            attention_weights).flatten(2)
        output = output.view(bs, self.num_bev_queue, num_query,
                             embed_dims).mean(1)
        output = self.output_proj(output)
