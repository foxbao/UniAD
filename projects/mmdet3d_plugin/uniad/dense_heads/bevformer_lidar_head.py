...
class BEVFormerLidarHead(BaseModule):
    ...
    def __init__(self,
        ...
        self.with_track_branch = with_track_branch
        self.track_steps = int(past_steps) + int(fut_steps)
        if self.with_track_branch and self.track_steps <= 0:
            raise ValueError('with_track_branch requires past_steps + '
                             'fut_steps > 0.')
        if self.with_track_branch and self.num_query is None:
            raise ValueError('with_track_branch requires num_query.')
        ...
        transformer.setdefault('type', 'LidarPerceptionTransformer')
        ...
        self.reg_branches = nn.ModuleList(
            [self._make_reg_branch(num_reg_fcs)
             for _ in range(self.num_decoder_layers)])
        if self.with_track_branch:
            self.past_traj_reg_branches = nn.ModuleList(
                [self._make_traj_branch(num_reg_fcs)
                 for _ in range(self.num_decoder_layers)])

        # ---- losses / assigner --------------------------------------------
        self.loss_cls = build_loss(self._strip_scope(loss_cls))
        self.loss_bbox = build_loss(self._strip_scope(loss_bbox))
        self.loss_iou = build_loss(self._strip_scope(loss_iou)) if loss_iou else None
    ...
    @staticmethod
    def _strip_scope(cfg: Optional[dict]) -> Optional[dict]:
        if cfg is None:
            return None
        cfg = dict(cfg)
        if isinstance(cfg.get('type'), str) and cfg['type'].startswith('mmdet.'):
            cfg['type'] = cfg['type'].split('.', 1)[1]
        return cfg
    ...
    def _make_traj_branch(self, num_fcs: int) -> nn.Sequential:
        layers = []
        for _ in range(num_fcs):
            layers.extend([
                nn.Linear(self.embed_dims, self.embed_dims),
        ...
        layers.append(nn.Linear(self.embed_dims, self.track_steps * 2))
        return nn.Sequential(*layers)
    ...
    def get_bev_features_trt(self,
                             lidar_bev: Tensor,
                             prev_bev: Tensor,
                             shift: Tensor,
                             use_prev_bev: Tensor) -> Tensor:
        batch_size, channels, bev_h, bev_w = lidar_bev.shape
        expected_shape = (self.lidar_in_channels, self.bev_h, self.bev_w)
        if (channels, bev_h, bev_w) != expected_shape:
            raise ValueError('lidar_bev shape mismatch for '
                             'BEVFormerLidarHeadTRTP: expected '
                             f'(B, {expected_shape[0]}, {expected_shape[1]}, '
                             f'{expected_shape[2]}), got '
                             f'{tuple(lidar_bev.shape)}.')
        expected_prev_shape = (
        ...
        bev_pos = self.positional_encoding(batch_size, lidar_bev.device,
                                           lidar_bev.dtype)
        return self.transformer.get_bev_features_trt(
            encoder_lidar_bev,
    ...
    def forward(self, feats, object_query_embeds: Optional[Tensor] = None,
        ...
        all_past_traj_preds = []
        query_feats = []
        last_ref_points = None
        for layer_id in range(hs.shape[0]):
            reference = init_reference if layer_id == 0 else inter_references[
                layer_id - 1]
            reference = inverse_sigmoid(reference)
            cls_score = self.cls_branches[layer_id](hs[layer_id])
            reg_raw = self.reg_branches[layer_id](hs[layer_id])
            bbox_pred = self._decode_regression(reg_raw, reference)
            all_cls_scores.append(cls_score)
            all_bbox_preds.append(bbox_pred)
            if self.with_track_branch:
                traj_pred = self.past_traj_reg_branches[layer_id](
                    hs[layer_id])
                traj_pred = traj_pred.view(
                    batch_size, hs.size(2), self.track_steps, 2)
                all_past_traj_preds.append(traj_pred)
            query_feats.append(hs[layer_id])
            last_ref_points = torch.cat([
                reg_raw[..., 0:2] + reference[..., 0:2],
                reg_raw[..., 4:5] + reference[..., 2:3],
            ],
                                        dim=-1)
        ...
        if self.with_track_branch:
            outs['all_past_traj_preds'] = torch.stack(all_past_traj_preds)
        return outs
    ...
    def get_targets(self, batch_cls_scores: Tensor, batch_bbox_preds: Tensor,
                    gt_bboxes_list: List, gt_labels_list: List[Tensor]
                    ) -> Tuple:
        results = [
    ...
    def loss_by_feat_single(self, batch_cls_scores: Tensor,
                            ...
                            gt_bboxes_list: List,
                            gt_labels_list: List[Tensor]) -> Tuple:
        (labels_list, label_weights_list, bbox_targets_list, bbox_weights_list,
         num_total_pos, num_total_neg) = self.get_targets(
             batch_cls_scores, batch_bbox_preds, gt_bboxes_list,
             gt_labels_list)
    ...
    def predict_by_feat(self,
                        ...
                        return_query_feats: bool = False) -> List[dict]:
        decoded = self.bbox_coder.decode(preds)
        ret_list = []
        for sample_idx, det in enumerate(decoded):
            boxes = det['bboxes']
            scores = det['scores']
            labels = det['labels']
            ...
            result = dict(
                boxes_3d=box_type_3d(
                    boxes.detach().cpu(),
                    box_dim=boxes.size(-1),
                    origin=(0.5, 0.5, 0.5)),
                scores_3d=scores.detach().cpu(),
                labels_3d=labels.detach().cpu())
            if return_query_feats and 'query_feats' in preds:
                bbox_index = det.get('bbox_index')
                if bbox_index is not None:
                    mask = det.get('mask')
                    if mask is not None:
                        bbox_index = bbox_index[mask]
                    result['query_feats'] = preds['query_feats'][
...
@HEADS.register_module()
class BEVFormerLidarHeadTRTP(BEVFormerLidarHead):
    """LiDAR BEVFormer head configured for TensorRT plugin export."""
