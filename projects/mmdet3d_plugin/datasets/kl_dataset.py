...
class KlBEVFormerDataset(KlDataset):
    ...
    def _extract_raw_meta(self, raw_index):
        ...
        return dict(
            ...
            # Deterministic hard-GT QA pairs generated by gen_vlm_qa.py.
            # Keep this in img_metas as CPU-only metadata, parallel to
            # gt_caption, so existing Collect3D configs remain unchanged.
            gt_qa=info.get('vlm_qa') or (info.get('geo_facts') or {}).get('qa'))
    ...
    def _union2one(self, queue, raw_meta):
        ...
        img_metas['gt_qa'] = raw_meta[-1].get('gt_qa')
    ...
    def _union2one(self, queue, raw_meta):
        ...
        for idx, (frame, meta) in enumerate(zip(queue, raw_meta)):
            ...
            if idx == len(queue) - 1:
                ...
                frame_meta['gt_qa'] = meta.get('gt_qa')
